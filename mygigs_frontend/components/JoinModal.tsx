...
export default function PaymentPage() {
  ...
  const checkTransactionStatus = async (id: string) => {
    ...
    try {
      const response = await fetch(`http://localhost:8000/core/check-status/${id}/`);
      if (!response.ok) {
      ...
      if (data.status === 'success' || data.status === 'failed') {
        ...
        setMessage(isSuccess ? 'Payment was successful! Redirecting you to your profile...' : 'Payment failed. Please try again.');
        setLoading(false);
        setPollingId(null);
          ...
          <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden transform transition-all duration-500 ease-out scale-100 opacity-100">
            ...
            <div className="p-8 md:p-10 space-y-6">
              ...
              <div className="text-center">
                <h1 className="text-3xl font-bold text-gray-800">Premium Access</h1>
                <p className="mt-2 text-sm text-gray-500">
                  Unlock unlimited gig postings for KES {formatPrice(amount)} annually.
                </p>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div>
                  ...
                  <input
                    ...
                    required
                    className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-green-500 transition-shadow"
                ...
                <div>
                  ...
                  <input
                    ...
                    required
                    min="1"
                    className="w-full px-4 py-3 border font-black border-gray-300 rounded-xl text-black bg-gray-50 focus:outline-none focus:ring-2 focus:ring-green-500 transition-shadow"
                    placeholder="e.g., 200"
                  />
                </div>
              ...
              {message && (
                <div className={`mt-6 p-4 text-sm rounded-xl text-center font-semibold ${
